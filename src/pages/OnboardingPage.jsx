...
import { Zap, Building2, ArrowRight, Sparkles } from 'lucide-react'
...
export default function OnboardingPage() {
  ...
  const tenantCode   = userTenant?.tenant_code ?? ''
  const tenantName   = userTenant?.tenant_name ?? ''

  const [companyName, setCompanyName] = useState(tenantName)
  const [loading,     setLoading]     = useState(false)

  useEffect(() => {
    if (setupComplete) navigate('/brand', { replace: true })
  ...
  function handleStart(e) {
    e.preventDefault()
    if (!companyName.trim() || !tenantCode) return
    setLoading(true)
    createTenant({ id: tenantCode, name: companyName.trim() })
      ...
      >
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          <p style={{ fontSize: 14, fontFamily: FONT_SORA, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6, maxWidth: 340 }}>
            Vas a configurar la aplicación móvil de mensajeros. El proceso toma menos de 5 minutos.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            { n: '02', label: 'Colores y tema' },
            { n: '03', label: 'Funcionalidades' },
            { n: '04', label: 'Países activos' },
          ].map(s => (
        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          <div style={{ marginBottom: 32 }}>
            ...
            <div style={{ fontSize: 12, fontFamily: FONT_MONO, color: 'rgba(255,255,255,0.3)', lineHeight: 1.5 }}>
              Confirmá el nombre que verán los mensajeros en la app.
            </div>
          </div>

          <form onSubmit={handleStart} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {/* Tenant code — solo lectura */}
            <div>
              <label style={{ display: 'block', fontSize: 9, fontWeight: 700, fontFamily: FONT_MONO, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 7 }}>
                Identificador del tenant
              </label>
              <div style={{
                padding: '11px 14px', borderRadius: 10,
                background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)',
                fontSize: 12, fontFamily: FONT_MONO, color: 'rgba(255,255,255,0.35)',
            ...
            {/* Nombre de empresa */}
            <div>
              <label style={{ display: 'block', fontSize: 9, fontWeight: 700, fontFamily: FONT_MONO, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 7 }}>
              ...
              <input
                ...
                style={{
                  ...
                  fontFamily: FONT_SORA, outline: 'none', boxSizing: 'border-box',
            ...
            <motion.button
              type="submit"
              disabled={loading || !companyName.trim() || !tenantCode}
              whileHover={!loading ? { scale: 1.01 } : {}}
              whileTap={!loading ? { scale: 0.99 } : {}}
              style={{
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
                padding: '14px', borderRadius: 12, border: 'none',
                background: !companyName.trim() ? 'rgba(232,23,93,0.3)' : ACCENT,
                color: '#fff', fontSize: 14, fontFamily: FONT_SORA, fontWeight: 700,
                cursor: !companyName.trim() || loading ? 'default' : 'pointer',
                boxShadow: companyName.trim() ? '0 4px 20px rgba(232,23,93,0.35)' : 'none',
